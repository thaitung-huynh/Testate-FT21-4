...
public class MazeParser {
    public static Maze parse(String text) {
        ...
        for (String nonEmptyLine : nonEmptyLines)
            if (nonEmptyLine.length() != nonEmptyLines.get(0).length())
        ...
        for (int r = 1; r < nonEmptyLines.size() - 1; r += 2) {
            ...
            for (int c = 1; c < row.length() - 1; c += 2) {
                ...
                if (row.charAt(c + 1) == ' ') result.tearDownThisWall(new Border(new Cell(r, c), Direction.EAST));
                if (row1.charAt(c) == ' ') result.tearDownThisWall(new Border(new Cell(r, c), Direction.SOUTH));
            }
        }
