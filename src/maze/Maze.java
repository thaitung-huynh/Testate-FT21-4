...
public class Maze {
    ...
    public Maze(int height, int width) {
        this.height = height;
        this.width = width;
        stoneInDir = new boolean[height][width][4];
    ...
    public void tearDownThisWall(Border border) {
        ...
        switch (dir) {
            ...
            case WEST -> {
                ...
                stoneInDir[hier.getRow() - 1][hier.getColumn()][E] = false;
            }
        }
    }
    ...
    public boolean isOuterWall(Border border) {
        ...
        Direction dir = border.getDir();
        if (cell.getRow() == 0 && dir == Direction.NORTH) return true;
        if (cell.getRow() == height - 1 && dir == Direction.SOUTH) return true;
        if (cell.getRow() == width - 1 && dir == Direction.EAST) return true;
        return cell.getRow() == 0 && dir == Direction.WEST;
    }
    ...
    public Set<Border> getWalls() {
        ...
        for (int x = 0; x < height; ++x)
            for (int y = 0; y < width; ++y) {
                if (stoneInDir[x][y][S]) walls.add(new Border(new Cell(x, y), Direction.SOUTH));
                if (stoneInDir[x][y][E]) walls.add(new Border(new Cell(x, y), Direction.EAST));
                if (stoneInDir[x][y][W]) walls.add(new Border(new Cell(x, y), Direction.WEST));
                if (stoneInDir[x][y][N]) walls.add(new Border(new Cell(x, y), Direction.NORTH));
            }
        return walls;
    }
